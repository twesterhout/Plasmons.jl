...
"""
struct ThreeBlockMatrix{M <: AbstractMatrix{<:Real}}
    block₁::M
    block₂::M
    block₃::M
end
...
function ThreeBlockMatrix(G::AbstractMatrix, n₁::Int, n₂::Int)
    ...
    n₁ >= 0 || throw(DimensionMismatch("Invalid 'n₁': $n₁"))
    n₂ >= 0 || throw(DimensionMismatch("Invalid 'n₂': $n₂"))
    if (n₁ + n₂) >= size(G, 1)
        throw(ErrorException("Overlapping zero regions are not yet supported: $n₁ + $n₂ >= $(size(G, 1))"))
    end
    ThreeBlockMatrix(
